...
import { Repeater as RudraLayoutRepeater, Box as RudraLayoutBox } from '@rudra-studio/rudra-layout';
import { Typography as RudraCoreTypography, Button as RudraCoreButton, Alert as RudraCoreAlert, Card as RudraCoreCard } from '@rudra-studio/rudra-core';
...
export default function CompiledModule(props) {
  ...
  const inputs = { "programmeSlug": programmeSlug, "initialSolveMode": initialSolveMode, "subjectSlug": subjectSlug, "pageMode": pageMode, "authenticated": authenticated, "locale": locale, "accessProfile": accessProfile, "subjectCards": subjectCards, "returnPath": returnPath, "autoSolveOnLoad": autoSolveOnLoad, "semesterSlug": semesterSlug, "subjectsLocked": subjectsLocked, "contentPreview": contentPreview };
  ...
  const [showNormalDiscovery, set_showNormalDiscovery] = useState(() => structuredClone(true));
  const state = { "subjectNavigationFailed": subjectNavigationFailed, "showDemoSolution": showDemoSolution, "showActionMessage": showActionMessage, "isSubjectNavigating": isSubjectNavigating, "showAccessHint": showAccessHint, "canOpenProfessorStudio": canOpenProfessorStudio, "demoSolutionTitle": demoSolutionTitle, "demoSolutionMode": demoSolutionMode, "demoSolutionText": demoSolutionText, "actionMessage": actionMessage, "problemLanguage": problemLanguage, "demoVisibleSteps": demoVisibleSteps, "problemText": problemText, "accessDecision": accessDecision, "contentPreviewDisplay": contentPreviewDisplay, "canLearn": canLearn, "demoStepIndex": demoStepIndex, "lastProblemControlId": lastProblemControlId, "showNormalDiscovery": showNormalDiscovery };
  ...
  const _setState = useCallback((name, value) => {
    switch (name) {
      ...
      case "showNormalDiscovery": { const next = typeof value === 'function' ? value(state.showNormalDiscovery) : value; state.showNormalDiscovery = next; set_showNormalDiscovery(next); return next; }
  ...
  const _setStatePath = useCallback((path, value) => {
    ...
    switch (root) {
      ...
      case "showNormalDiscovery": _setState("showNormalDiscovery", updateNested); return value;
      default: return value;
    }
  }, [_setState]);
...
function normalizeContentPreview(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value) || !Object.keys(value).length) return { enabled: false, valid: false, showNormal: true };
  ...
  return { enabled: true, valid: true, showNormal: false, schemaVersion: 1,
...
try { return normalizeContentPreview(inputs.contentPreview); } catch { return { enabled: true, valid: false, showNormal: false, message: 'This preview is incomplete or invalid. Return to Studio and prepare it again.' }; }
      })();
    ...
    _setState("showNormalDiscovery", stepResults.preview_validate.showNormal);
...
return {
  ...
  return (
    <div ref={wrapperRef} className="rudra-module-wrapper">
      ...
      {isVisibleValue(((_bindingValue) => _bindingValue === undefined ? false : _bindingValue)(contentPreviewDisplay?.valid)) && (<>      <ChalkmindMathBlackboardLesson id="content_preview_board" autoAdvance={false} learningGoal={((_bindingValue) => _bindingValue === undefined ? "" : _bindingValue)(contentPreviewDisplay?.lesson?.learningGoal)} problemLabel={((_bindingValue) => _bindingValue === undefined ? "" : _bindingValue)(contentPreviewDisplay?.lesson?.problemLabel)} problemStatement={((_bindingValue) => _bindingValue === undefined ? "" : _bindingValue)(contentPreviewDisplay?.lesson?.problemStatement)} popupInitiallyOpen={false} steps={((_bindingValue) => _bindingValue === undefined ? [] : _bindingValue)(contentPreviewDisplay?.boardSteps)} title={((_bindingValue) => _bindingValue === undefined ? "" : _bindingValue)(contentPreviewDisplay?.lesson?.title)} playing={false} captionsEnabled={true} />
</>)}
</RudraLayoutBox>
</>)}
      {isVisibleValue(((_bindingValue) => _bindingValue === undefined ? true : _bindingValue)(showNormalDiscovery)) && (<>      <RudraLayoutBox id="content_preview_normal_hero" className="block">      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraLayoutBox id="hero" className="block rs-hero">      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraLayoutBox id="hero_inner" className="grid rs-hero-inner">      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraLayoutBox id="copy" className="flex flex-col rs-stack">      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraCoreTypography id="kicker" className="rs-kicker" as="p" content={((_bindingValue) => _bindingValue === undefined ? "A mathematics studio for college" : _bindingValue)(_scope?.i18n?.kicker)} />
</>)}
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraCoreTypography id="title" className="rs-title" customColor="#effff9" as="h2" content={((_bindingValue) => _bindingValue === undefined ? "See the reasoning. Steer the lesson." : _bindingValue)(_scope?.i18n?.title)} />
</>)}
      ...
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraCoreButton id="prof_cta" variant="outline" onAction={(...eventArgs) => _callAction("openProfessorStudio", {}, eventArgs)} size="lg" label="Professor Studio" theme="dark" />
</>)}
</RudraLayoutBox>
</>)}
</RudraLayoutBox>
      ...
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraLayoutBox id="problem_actions" className="grid rs-actions rs-solution-controls">      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraCoreButton id="solve_now" id="scholar-demo-quick" label={((_bindingValue) => _bindingValue === undefined ? "Show answer" : _bindingValue)(_scope?.i18n?.solveNow)} theme="light" variant="primary" onAction={(...eventArgs) => _callAction("showQuickSolution", {}, eventArgs)} aria-pressed={(((value) => { return value === 'answer'; })(((_bindingValue) => _bindingValue === undefined ? "answer" : _bindingValue)(demoSolutionMode)))} additionalAttributes={{}} type="button" aria-controls="scholar-demo-solution" />
</>)}
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraCoreButton id="steps" id="scholar-demo-detailed" label={((_bindingValue) => _bindingValue === undefined ? "Show detailed steps" : _bindingValue)(_scope?.i18n?.solveSteps)} variant="outline" aria-pressed={(((value) => { return value === 'steps'; })(((_bindingValue) => _bindingValue === undefined ? "answer" : _bindingValue)(demoSolutionMode)))} type="button" theme="light" onAction={(...eventArgs) => _callAction("showDetailedSolution", {}, eventArgs)} aria-controls="scholar-demo-solution" additionalAttributes={{}} />
</>)}
      ...
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraCoreButton id="demo_step_pick_5" className="rs-step-dot" aria-pressed={(((value) => { return value === 5; })(((_bindingValue) => _bindingValue === undefined ? 1 : _bindingValue)(demoStepIndex)))} id="scholar-demo_step_pick_5" type="button" label="5" value={5} onAction={(...eventArgs) => _callAction("selectDemoSolutionStep", {}, eventArgs)} ariaLabel={((_bindingValue) => _bindingValue === undefined ? "Step 5: Check both equations" : _bindingValue)(_scope?.i18n?.demoStepLabel5)} />
</>)}
</RudraLayoutBox>
</>)}
...
})(value).result; })(((_bindingValue) => _bindingValue === undefined ? "en" : _bindingValue)(inputs?.locale)))} as="p" />
</>)}
...
</>} variant="ghost" disabled={true} onAction={(...eventArgs) => _callAction("requestImage", {}, eventArgs)} ariaLabel="Image problem upload is locked until a post-release update" additionalAttributes={{"disabled":true,"title":"Planned for a post-release update"}} id="scholar-image-upload-locked" label="Upload an image · Coming soon" theme="light" />
</>)}
  ...
  return cards;
...
</>} disabled={((_bindingValue) => _bindingValue === undefined ? false : _bindingValue)(_scope?.item?.locked)} size="md" loading={((_bindingValue) => _bindingValue === undefined ? false : _bindingValue)(isSubjectNavigating)} variant="outline" onAction={(...eventArgs) => _callAction("openDiscoverySubject", {}, eventArgs)} fullWidth={true} type="button" label={((_bindingValue) => _bindingValue === undefined ? "Open subject" : _bindingValue)(_scope?.item?.actionLabel)} theme="auto" value={((_bindingValue) => _bindingValue === undefined ? "" : _bindingValue)(_scope?.item?.redirectionLink)} />
</>)}
</RudraCoreCard>
</>)}
      ...
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraCoreButton id="discrete_go" onAction={(...eventArgs) => _callAction("navigate", {"path": "/browse/engineering/semester-1/discrete-mathematics"}, eventArgs)} fullWidth={true} size="md" label="Open subject" theme="auto" variant="outline" />
</>)}
</RudraCoreCard>
</>)}
</>); })(); }}</RudraLayoutRepeater>
