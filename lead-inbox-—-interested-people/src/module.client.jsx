...
import { Header as RudraWidgetsHeader, MetricCard as RudraWidgetsMetricCard, DataTable as RudraWidgetsDataTable } from '@rudra-studio/rudra-widgets';
...
import { Grid as RudraLayoutGrid, Box as RudraLayoutBox, Flex as RudraLayoutFlex, ScrollArea as RudraLayoutScrollArea, Container as RudraLayoutContainer, Section as RudraLayoutSection } from '@rudra-studio/rudra-layout';
...
export default function CompiledModule(props) {
  ...
  const inputs = { "where": where, "to": to, "intent": intent, "status": status, "source": source, "from": from };
  ...
  const _setState = useCallback((name, value) => {
    switch (name) {
      case "leadAnalytics": set_leadAnalytics(value); return value;
      case "isLoadingLeads": set_isLoadingLeads(value); return value;
      case "leads": set_leads(value); return value;
  ...
  const _setStatePath = useCallback((path, value) => {
    ...
    switch (root) {
      case "leadAnalytics": set_leadAnalytics(updateNested); return value;
      case "isLoadingLeads": set_isLoadingLeads(updateNested); return value;
      case "leads": set_leads(updateNested); return value;
      default: return value;
    }
  }, [_setState]);
  ...
  return (
    <div ref={wrapperRef} className="rudra-module-wrapper">
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraLayoutBox id="leads_root" data-theme={((_bindingValue) => _bindingValue === undefined ? "light" : _bindingValue)($theme)} className="flex min-h-screen w-full flex-col overflow-x-hidden leads-shell" as="main">      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraWidgetsHeader id="leads_header" className={`${getResponsiveProp({sm: 'leads-header h-16 px-4 border-white/10', md: 'leads-header h-18 px-8 border-white/10', lg: 'leads-header h-20 px-12 border-white/10'}) || ''}`} navItemClassName="text-xs font-black uppercase tracking-[0.12em]" mobileBreakpoint="md" theme="auto" title="SIGNAL/ROOM" sticky={true} navItems={[{"href":"?status=","id":"all","label":"All leads"},{"href":"?status=new","id":"new","label":"New"},{"href":"?intent=job","id":"jobs","label":"Jobs"},{"href":"?intent=collaboration","id":"collabs","label":"Collaborations"}]} />
</>)}
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraLayoutSection id="leads_main" className={`${getResponsiveProp({sm: 'px-4 py-14', md: 'px-8 py-20', lg: 'px-12 py-24'}) || ''}`} as="section">      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraLayoutContainer id="leads_container" className="w-full max-w-[1500px]" centered={true} maxWidth="full">      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraLayoutFlex id="leads_meta" className="border-t border-[#6dffb8]/25 pt-3" justify="between" direction="horizontal" as="div" gap="4" wrap={true} align="center">      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraCoreTypography id="leads_kicker" className="leads-signal ml-5 text-[10px] font-bold tracking-[.18em] text-[#6dffb8]" content="LIVE INTAKE · SERVER-RENDERED · FILTERED" as="p" />
</>)}
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraCoreBadge id="leads_badge" className="rounded-full border border-[#6dffb8]/35 bg-[#6dffb8]/10 text-[#6dffb8]" size="sm" label="SYSTEM ONLINE" variant="solid" />
</>)}
</RudraLayoutFlex>
</>)}
      ...
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraLayoutGrid id="leads_kpi_grid" className={`grid flex ${getResponsiveProp({sm: 'grid grid-cols-1 gap-4', md: 'grid grid-cols-2 gap-4', lg: 'grid grid-cols-4 gap-4'}) || ''}`} as="div">      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraWidgetsMetricCard id="leads_metric_total" trendClassName={`${getResponsiveProp({sm: 'leads-metric-trend'}) || ''}`} chartClassName={`${getResponsiveProp({sm: 'leads-metric-chart'}) || ''}`} descriptionClassName={`${getResponsiveProp({sm: 'leads-metric-description'}) || ''}`} valueClassName={`${getResponsiveProp({sm: 'leads-metric-value'}) || ''}`} className={`${getResponsiveProp({sm: 'leads-metric'}) || ''}`} labelClassName={`${getResponsiveProp({sm: 'leads-metric-label'}) || ''}`} value={((_bindingValue) => _bindingValue === undefined ? 0 : _bindingValue)(leadAnalytics?.total_leads)} positiveColor="#6dffb8" label="Total leads" showChart={false} description="Records matching the active filter" neutralColor="#62d9ff" style={{"background":"#0d1319","border":"1px solid rgba(98,217,255,.18)","borderRadius":"20px","padding":"20px"}} showTrend={false} negativeColor="#fb7185" accentColor="#62d9ff" />
</>)}
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraWidgetsMetricCard id="leads_metric_new" labelClassName={`${getResponsiveProp({sm: 'leads-metric-label'}) || ''}`} trendClassName={`${getResponsiveProp({sm: 'leads-metric-trend'}) || ''}`} chartClassName={`${getResponsiveProp({sm: 'leads-metric-chart'}) || ''}`} descriptionClassName={`${getResponsiveProp({sm: 'leads-metric-description'}) || ''}`} valueClassName={`${getResponsiveProp({sm: 'leads-metric-value'}) || ''}`} className={`${getResponsiveProp({sm: 'leads-metric'}) || ''}`} trend="up" accentColor="#6dffb8" description="Awaiting first response" negativeColor="#fb7185" showChart={false} trendLabel="current queue" neutralColor="#62d9ff" label="New leads" value={((_bindingValue) => _bindingValue === undefined ? 0 : _bindingValue)(leadAnalytics?.new_leads)} showTrend={true} positiveColor="#6dffb8" trendValue="Live" style={{"background":"#0d1319","border":"1px solid rgba(98,217,255,.18)","borderRadius":"20px","padding":"20px"}} />
</>)}
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraWidgetsMetricCard id="leads_metric_jobs" valueClassName={`${getResponsiveProp({sm: 'leads-metric-value'}) || ''}`} labelClassName={`${getResponsiveProp({sm: 'leads-metric-label'}) || ''}`} trendClassName={`${getResponsiveProp({sm: 'leads-metric-trend'}) || ''}`} chartClassName={`${getResponsiveProp({sm: 'leads-metric-chart'}) || ''}`} descriptionClassName={`${getResponsiveProp({sm: 'leads-metric-description'}) || ''}`} className={`${getResponsiveProp({sm: 'leads-metric'}) || ''}`} label="Job intent" value={((_bindingValue) => _bindingValue === undefined ? 0 : _bindingValue)(leadAnalytics?.job_leads)} showTrend={false} accentColor="#a78bfa" negativeColor="#fb7185" positiveColor="#6dffb8" showChart={false} description="Career and hiring signals" neutralColor="#62d9ff" style={{"background":"#0d1319","border":"1px solid rgba(98,217,255,.18)","borderRadius":"20px","padding":"20px"}} />
</>)}
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraWidgetsMetricCard id="leads_metric_sources" valueClassName={`${getResponsiveProp({sm: 'leads-metric-value'}) || ''}`} labelClassName={`${getResponsiveProp({sm: 'leads-metric-label'}) || ''}`} trendClassName={`${getResponsiveProp({sm: 'leads-metric-trend'}) || ''}`} className={`${getResponsiveProp({sm: 'leads-metric'}) || ''}`} chartClassName={`${getResponsiveProp({sm: 'leads-metric-chart'}) || ''}`} descriptionClassName={`${getResponsiveProp({sm: 'leads-metric-description'}) || ''}`} style={{"background":"#0d1319","border":"1px solid rgba(98,217,255,.18)","borderRadius":"20px","padding":"20px"}} accentColor="#fbbf24" description="Distinct acquisition channels" neutralColor="#62d9ff" label="Active sources" positiveColor="#6dffb8" showChart={false} value={((_bindingValue) => _bindingValue === undefined ? 0 : _bindingValue)(leadAnalytics?.active_sources)} showTrend={false} negativeColor="#fb7185" />
</>)}
</RudraLayoutGrid>
</>)}
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraLayoutGrid id="leads_insight_grid" className={`${getResponsiveProp({sm: 'mt-4 grid grid-cols-1 gap-4', md: 'mt-4 grid grid-cols-2 gap-4', lg: 'mt-4 grid grid-cols-3 gap-4'}) || ''}`} as="div">      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraWidgetsMetricCard id="leads_metric_volume" className={`${getResponsiveProp({sm: 'leads-metric'}) || ''}`} labelClassName={`${getResponsiveProp({sm: 'leads-metric-label'}) || ''}`} trendClassName={`${getResponsiveProp({sm: 'leads-metric-trend'}) || ''}`} chartClassName={`${getResponsiveProp({sm: 'leads-metric-chart'}) || ''}`} descriptionClassName={`${getResponsiveProp({sm: 'leads-metric-description'}) || ''}`} valueClassName={`${getResponsiveProp({sm: 'leads-metric-value'}) || ''}`} chartData={((_bindingValue) => _bindingValue === undefined ? [] : _bindingValue)(leadAnalytics?.daily_lead_counts)} chartType="area" showTrend={true} trendLabel="received in last 7 days" trendValue={((_bindingValue) => _bindingValue === undefined ? 0 : _bindingValue)(leadAnalytics?.leads_last_7_days)} positiveColor="#6dffb8" label="Lead volume" accentColor="#62d9ff" chartHeight={82} description="Daily arrivals represented in the current result set" style={{"background":"#0d1319","border":"1px solid rgba(98,217,255,.18)","borderRadius":"20px","padding":"20px"}} trend="neutral" value={((_bindingValue) => _bindingValue === undefined ? 0 : _bindingValue)(leadAnalytics?.total_leads)} showChart={true} neutralColor="#62d9ff" negativeColor="#fb7185" />
</>)}
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraWidgetsMetricCard id="leads_metric_intents" labelClassName={`${getResponsiveProp({sm: 'leads-metric-label'}) || ''}`} trendClassName={`${getResponsiveProp({sm: 'leads-metric-trend'}) || ''}`} chartClassName={`${getResponsiveProp({sm: 'leads-metric-chart'}) || ''}`} descriptionClassName={`${getResponsiveProp({sm: 'leads-metric-description'}) || ''}`} valueClassName={`${getResponsiveProp({sm: 'leads-metric-value'}) || ''}`} className={`${getResponsiveProp({sm: 'leads-metric'}) || ''}`} style={{"background":"#0d1319","border":"1px solid rgba(98,217,255,.18)","borderRadius":"20px","padding":"20px"}} trend="neutral" value={((_bindingValue) => _bindingValue === undefined ? 0 : _bindingValue)(leadAnalytics?.project_leads)} chartType="line" negativeColor="#fb7185" positiveColor="#6dffb8" chartData={((_bindingValue) => _bindingValue === undefined ? [0, 0, 0] : _bindingValue)(leadAnalytics?.intent_counts)} showTrend={true} chartHeight={82} label="Intent mix" trendValue={((_bindingValue) => _bindingValue === undefined ? 0 : _bindingValue)(leadAnalytics?.job_leads)} accentColor="#a78bfa" description="Project leads shown as the headline value" neutralColor="#62d9ff" showChart={true} trendLabel="job leads" />
</>)}
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraWidgetsMetricCard id="leads_metric_contact" trendClassName={`${getResponsiveProp({sm: 'leads-metric-trend'}) || ''}`} className={`${getResponsiveProp({sm: 'leads-metric'}) || ''}`} chartClassName={`${getResponsiveProp({sm: 'leads-metric-chart'}) || ''}`} descriptionClassName={`${getResponsiveProp({sm: 'leads-metric-description'}) || ''}`} valueClassName={`${getResponsiveProp({sm: 'leads-metric-value'}) || ''}`} labelClassName={`${getResponsiveProp({sm: 'leads-metric-label'}) || ''}`} negativeColor="#fb7185" label="Contact progress" style={{"background":"#0d1319","border":"1px solid rgba(98,217,255,.18)","borderRadius":"20px","padding":"20px"}} showChart={false} accentColor="#6dffb8" description="Share moved beyond new status" value={((_bindingValue) => _bindingValue === undefined ? 0 : _bindingValue)(leadAnalytics?.contact_progress)} suffix="%" trendLabel="progressed leads" trendValue={((_bindingValue) => _bindingValue === undefined ? 0 : _bindingValue)(leadAnalytics?.progressed_leads)} showTrend={true} neutralColor="#62d9ff" positiveColor="#6dffb8" trend="up" />
</>)}
</RudraLayoutGrid>
</>)}
</RudraLayoutBox>
</>)}
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraCoreSurface id="leads_filters" className={`${getResponsiveProp({sm: 'leads-panel mt-12 rounded-[1.25rem] border border-[#6dffb8]/20 bg-[#0d1319] p-5 text-white', md: 'leads-panel mt-14 rounded-[1.5rem] border border-[#6dffb8]/20 bg-[#0d1319] p-7 text-white', lg: 'leads-panel mt-16 rounded-[1.5rem] border border-[#6dffb8]/20 bg-[#0d1319] p-8 text-white'}) || ''}`} theme="auto" as="section">      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraCoreTypography id="leads_filters_label" className="text-[10px] font-bold tracking-[.18em] text-[#6dffb8]" as="h2" content="ACTIVE FILTERS" />
</>)}
      ...
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraCoreBadge id="leads_filter_to" className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-white" label={to} variant="outline" size="md" />
</>)}
</RudraLayoutFlex>
</>)}
</RudraCoreSurface>
</>)}
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraCoreSurface id="leads_table_surface" className={`${getResponsiveProp({sm: 'leads-table-glow mt-6 overflow-hidden rounded-[1.25rem] p-4', md: 'leads-table-glow rounded-[1.5rem] p-6', lg: 'leads-table-glow rounded-[1.5rem] p-8'}) || ''}`} as="section" theme="auto">      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraLayoutFlex id="leads_table_head" className="mb-6 border-b border-white/10 pb-5" justify="between" direction="horizontal" as="div" gap="4" wrap={true} align="end">      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraCoreTypography id="leads_table_title" className={`${getResponsiveProp({sm: 'leads-display text-3xl text-white', md: 'leads-display text-4xl text-white', lg: 'leads-display text-4xl text-white'}) || ''}`} content="SIGNAL LOG" as="h2" />
</>)}
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraCoreTypography id="leads_table_hint" className="text-xs font-bold text-[#62d9ff]/70" content="Search, filter and paginate" as="p" />
</>)}
</RudraLayoutFlex>
</>)}
      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraLayoutScrollArea id="leads_table_scroll" className="w-full overflow-x-auto" orientation="horizontal">      {isVisibleValue(getResponsiveProp({ "lg": true, "md": true, "sm": true })) && (<>      <RudraWidgetsDataTable id="leads_table" className="min-w-[880px]" globalFilterPlaceholder="Search leads" data={leads} mode="pagination" columns={[{"accessorKey":"full_name","header":"Name"},{"accessorKey":"email","header":"Email"},{"accessorKey":"company","header":"Company"},{"accessorKey":"intent","header":"Intent"},{"accessorKey":"source","header":"Source"},{"accessorKey":"status","header":"Status"},{"accessorKey":"created_at","header":"Received"}]} pageSize={10} />
</>)}
